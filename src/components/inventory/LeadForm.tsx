...
export function LeadForm({ vehicleName, inquiryType, onInquiryTypeChange }: LeadFormProps) {
  ...
  if (submitted) {
    return (
      <div className="border border-lime-400/30 bg-lime-400/10 p-5" role="status" aria-live="polite">
        <h3 className="text-lg font-semibold text-lime-100">Request received</h3>
  ...
  return (
    <form className="premium-panel space-y-4 p-5" onSubmit={handleSubmit} noValidate>
      <div>
        <h3 className="text-xl font-semibold text-stone-50">{inquiryLabels[inquiryType]}</h3>
      ...
      <label className="block">
        ...
        <input
          ...
          type="text"
        />
        {fieldErrors.name ? <p className="mt-2 text-sm font-semibold text-red-300" id="lead-name-error">{fieldErrors.name}</p> : null}
      </label>
      <label className="block">
        ...
        <input
          ...
          type="text"
        />
        {fieldErrors.contact ? <p className="mt-2 text-sm font-semibold text-red-300" id="lead-contact-error">{fieldErrors.contact}</p> : null}
      </label>
      <label className="block">
